    ...
    for (auto const &dir : directions) {
      ...
      if (!walls.contains(path.position + dir) &&
        ...
        if (dir == newPath.direction) {
          ...
          newPath.orderedPath.push_back(newPath.position);
        }
        if (newPath.score <= lowScore) {
          ...
          if (newPath.position == end) {
            ...
            PositionType pos = start;
            for (auto const &step : newPath.orderedPath) {
              PositionType dir = pos - step;
              PositionType newPos = pos;
              if (dir.col == 0 && dir.row < 0) {
                for (int i = 0; i >= dir.row; i--) {
                  newPos = step + PositionType{dir.row - i, dir.col};
                  bestPath.insert(newPos);
                }
              } else if (dir.col == 0 && dir.row > 0) {
                for (int i = 0; i <= dir.row; i++) {
                  newPos = step + PositionType{dir.row - i, dir.col};
                  bestPath.insert(newPos);
                }
              } else if (dir.col < 0 && dir.row == 0) {
                for (int i = 0; i >= dir.col; i--) {
                  newPos = step + PositionType{dir.row, dir.col - i};
                  bestPath.insert(newPos);
                }
              } else if (dir.col > 0 && dir.row == 0) {
                for (int i = 0; i <= dir.col; i++) {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      if (walls.contains(PositionType{i, j})) {
        std::cout << "#";
      } else if (bestPath.contains(PositionType{i, j})) {
        std::cout << "o";
      } else {
        std::cout << ".";
      }

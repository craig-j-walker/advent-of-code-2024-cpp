...
export module part_two_d2;

export namespace part_two_d2 {
int64_t safe(const std::string_view input) {
  ...
  for (auto &report : reports) {
    ...
    for (int j = -1; j != reportTokens.size(); j++) {
      int i = 0;
      std::vector<std::string_view> filteredTokens = reportTokens;
      if (j != -1) {
        filteredTokens.erase(filteredTokens.begin() + j);
      }
